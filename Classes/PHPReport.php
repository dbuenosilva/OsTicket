...
class PHPReport {
	...
	private function createTemplate()
	{
		...
		$this->objWorksheet->getPageSetup()->setOrientation(PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
        ...
        foreach($this->_data as $data)
        {
            ...
            if(isset($data['header']))
            {
                ...
                foreach($data['header'] as $k=>$v)
                {
                    ...
                    if(isset($config['header'][$k]['valign']))
                        $this->objWorksheet->getStyleByColumnAndRow($colIndex,$nextRow)->getAlignment()->setVertical($config['header'][$k]['valign']);
                }
            ...
            if(count($data['data'])>0)
            {
                ...
                foreach($singleDataRow as $k=>$v)
                {
                    ...
                    if(isset($config['data'][$k]['valign']))
                        $this->objWorksheet->getStyleByColumnAndRow($colIndex,$nextRow)->getAlignment()->setVertical($config['data'][$k]['valign']);
                }
            }
