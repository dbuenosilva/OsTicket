    ...
    if($ticket && $ticket->getId()) {
        ...
        case 'reply':
            if(!$thisstaff->canPostReply())
				...
				if(!$_POST['1entrada'])
                    $errors['1entrada']=__('Data não preenchida');
				
				if($_POST['hentra'] === $_POST['hsai'] )
                    $errors['hsai']=__('Entrada não pode ser igual a saída.');
				if($_POST['hsai'] <= $_POST['hentra'] )
					$errors['hsai']=__('Saída não pode ser menor do que a entrada.');
				
				if($_POST['hentra2'] != '00:00')
					if($_POST['hentra2'] === $_POST['hsai2'] )
                    $errors['hsai2']=__('Entrada não pode ser igual a saída.');
					if($_POST['hsai2'] <= $_POST['hentra2'] && $_POST['hentra2'] != '00:00')
					$errors['hsai2']=__('Saída não pode ser menor do que a entrada.');
				
				if($_POST['hentra3'] != '00:00' )
					if($_POST['hsai3'] === $_POST['hentra3'] )
                    $errors['hsai3']=__('Entrada não pode ser igual a saída.');
					if($_POST['hsai3'] <= $_POST['hentra3'] && $_POST['hentra3'] != '00:00')
					$errors['hsai2']=__('Saída não pode ser menor do que a entrada.');
				
				if(!$_POST['tipoatende'])
                    $errors['tipoatende']=__('Tipo de atendimento não preenchido'); 
				
                //Usa travas para evitar respostas duplas
